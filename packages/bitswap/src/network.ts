...
import {
  ...
  DEFAULT_MAX_OUTGOING_MESSAGE_SIZE,
  DEFAULT_MAX_PROVIDERS_PER_REQUEST,
  DEFAULT_MESSAGE_RECEIVE_TIMEOUT,
  DEFAULT_MESSAGE_SEND_CONCURRENCY,
  DEFAULT_RUN_ON_TRANSIENT_CONNECTIONS,
} from "./constants.js";
...
export type BitswapNetworkProgressEvents = ProgressEvent<
...
export type BitswapNetworkWantProgressEvents =
  ...
  | ProgressEvent<
      "bitswap:network:send-wantlist:error",
      { peer: PeerId; error: Error }
    >
...
interface SendMessageJobOptions
...
export class Network extends TypedEventEmitter<NetworkEvents> {
  ...
  constructor(components: NetworkComponents, init: NetworkInit = {}) {
    ...
    this.maxInboundStreams =
      init.maxInboundStreams ?? DEFAULT_MAX_INBOUND_STREAMS;
    this.maxOutboundStreams =
      init.maxOutboundStreams ?? DEFAULT_MAX_OUTBOUND_STREAMS;
    this.messageReceiveTimeout =
      init.messageReceiveTimeout ?? DEFAULT_MESSAGE_RECEIVE_TIMEOUT;
    this.runOnTransientConnections =
      init.runOnTransientConnections ?? DEFAULT_RUN_ON_TRANSIENT_CONNECTIONS;
    this.maxIncomingMessageSize =
      init.maxIncomingMessageSize ?? DEFAULT_MAX_OUTGOING_MESSAGE_SIZE;
    this.maxOutgoingMessageSize =
      init.maxOutgoingMessageSize ??
      init.maxIncomingMessageSize ??
      DEFAULT_MAX_INCOMING_MESSAGE_SIZE;
    this.metrics = {
      blocksSent: components.metrics?.registerCounter(
        "helia_bitswap_sent_blocks_total",
      ),
      dataSent: components.metrics?.registerCounter(
        "helia_bitswap_sent_data_bytes_total",
      ),
    };

    this.sendQueue = new PeerQueue({
      concurrency:
        init.messageSendConcurrency ?? DEFAULT_MESSAGE_SEND_CONCURRENCY,
  ...
  _onStream(info: IncomingStreamData): void {
    ...
    Promise.resolve()
      .then(async () => {
        this.log(
          "incoming new bitswap %s stream from %p",
          stream.protocol,
        ...
        const abortListener = (): void => {
          if (stream.status === "open") {
            stream.abort(
              new CodeError(
                `Incoming Bitswap stream timed out after ${this.messageReceiveTimeout}ms`,
                "ERR_TIMEOUT",
              ),
            );
          } else {
          ...
          async (source) => {
            for await (const data of source) {
              try {
                ...
                this.log(
                  "incoming new bitswap %s message from %p on stream",
                  stream.protocol,
                  connection.remotePeer,
                  stream.id,
                );
                ...
                this.log.error(
                  "error reading incoming bitswap message from %p on stream",
                  connection.remotePeer,
                  stream.id,
                  err,
                );
      ...
      .catch((err) => {
        this.log.error(
          "error handling incoming stream from %p",
          connection.remotePeer,
          err,
        );
        stream.abort(err);
  ...
  async *findProviders(
    ...
    options?.onProgress?.(
      new CustomProgressEvent<CID>("bitswap:network:find-providers", cid),
    );
  ...
  async findAndConnect(cid: CID, options?: WantOptions): Promise<void> {
    await drain(
      map(
        take(
          this.findProviders(cid, options),
  ...
  async sendMessage(
    ...
    if (existingJob != null) {
      existingJob.options.message = mergeMessages(
        existingJob.options.message,
        message,
      );
    ...
    await this.sendQueue.add(
      async (options) => {
        ...
        options?.onProgress?.(
          new CustomProgressEvent<PeerId>(
        ...
        try {
          ...
          options?.onProgress?.(
            new CustomProgressEvent<{ peer: PeerId; error: Error }>(
              "bitswap:network:send-wantlist:error",
  ...
  async connectTo(
    peer: PeerId,
    options?: AbortOptions & ProgressOptions<BitswapNetworkProgressEvents>,
  ): Promise<Connection> {
    ...
    options?.onProgress?.(
      new CustomProgressEvent<PeerId>("bitswap:network:dial", peer),
    );
    ...
    const [connection] = await Promise.all([
      ...
      raceEvent(this.libp2p, "peer:identify", options?.signal, {
        filter: (evt: CustomEvent<IdentifyResult>): boolean => {
          ...
          throw new CodeError(
            `${peer} did not support ${BITSWAP_120}`,
            "ERR_BITSWAP_UNSUPPORTED_BY_PEER",
          );
        },
      }),
    ]);
