...
export interface WantOptions
  extends AbortOptions,
    ProgressOptions<BitswapNetworkWantProgressEvents> {
  /**
...
export class WantList
  extends TypedEventEmitter<WantListEvents>
  implements Startable, TypedEventTarget<WantListEvents>
{
  /**
  ...
  constructor(components: WantListComponents, init: WantListInit = {}) {
    ...
    this.sendMessagesDelay =
      init.sendMessagesDelay ?? DEFAULT_MESSAGE_SEND_DELAY;
    ...
    this.network.addEventListener("bitswap:message", (evt) => {
      this.receiveMessage(evt.detail.peer, evt.detail.message).catch((err) => {
        this.log.error(
          "error receiving bitswap message from %p",
          evt.detail.peer,
          err,
        );
      });
    ...
    this.network.addEventListener("peer:connected", (evt) => {
      this.peerConnected(evt.detail).catch((err) => {
        this.log.error(
          "error processing newly connected bitswap peer %p",
          evt.detail,
          err,
        );
      });
  ...
  private async addEntry(
    cid: CID,
    options: WantOptions & { wantType: WantType.WantBlock },
  ): Promise<WantBlockResult>;
  private async addEntry(
    cid: CID,
    options: WantOptions & { wantType: WantType.WantHave },
  ): Promise<WantPresenceResult>;
  private async addEntry(
    cid: CID,
    options: WantOptions & { wantType: WantType },
  ): Promise<WantBlockResult | WantPresenceResult> {
    ...
    if (
      entry.wantType === WantType.WantHave &&
      options.wantType === WantType.WantBlock
    ) {
    ...
    try {
      if (options.wantType === WantType.WantBlock) {
        const event = await raceEvent<CustomEvent<WantBlockResult>>(
          this,
          "block",
          options?.signal,
          {
            filter: (event) => {
              return uint8ArrayEquals(
                ...
                event.detail.cid.multihash.digest,
              );
            },
      ...
      const event = await raceEvent<CustomEvent<WantPresenceResult>>(
        this,
        "presence",
        options?.signal,
        {
  ...
  async wantSessionPresence(
    ...
    const event = await raceEvent<
      CustomEvent<WantHaveResult | WantDontHaveResult>
    >(this, "presence", options.signal, {
      filter: (event) => {
        return (
          peerId.equals(event.detail.sender) &&
          uint8ArrayEquals(
            cid.multihash.digest,
            event.detail.cid.multihash.digest,
          )
  ...
  async receivedBlock(
    cid: CID,
    options: ProgressOptions<BitswapNotifyProgressEvents> & AbortOptions,
  ): Promise<void> {
    const cidStr = uint8ArrayToString(cid.multihash.bytes, "base64");
  ...
  private async receiveMessage(
    ...
    this.log(
      "received message from %p with %d blocks",
      sender,
    ...
    for (const block of message.blocks) {
      ...
      const hasher =
        hashAlg === sha256.code
          ? sha256
          : await this.hashLoader?.getHasher(hashAlg);
