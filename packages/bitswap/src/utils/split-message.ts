...
export function* splitMessage(
  ...
  while (true) {
    ...
    let { added, hasMore, newSize } = addToMessage(
      blocks,
      subMessage.blocks,
      blocksIndex,
      maxSize,
      size,
      calculateEncodedBlockSize,
    );
    ...
    ({ added, hasMore, newSize } = addToMessage(
      blockPresences,
      subMessage.blockPresences,
      blockPresencesIndex,
      maxSize,
      size,
      calculateEncodedBlockPresenceSize,
    ));
    ...
    ({ added, hasMore, newSize } = addToMessage(
      wantListEntries,
      subMessage.wantlist.entries,
      wantListIndex,
      maxSize,
      size,
      calculateEncodedWantlistEntrySize,
    ));
    ...
    doneSending =
      !haveMoreBlocks && !haveMorePresences && !haveMoreWantlistEntries;
...
function addToMessage<T>(
  input: T[],
  output: T[],
  start: number,
  maxSize: number,
  size: number,
  calculateSize: (arg: T) => number,
): AddResult {
  let added = 0;
  let hasMore = false;
  ...
  for (let i = start; i < input.length; i++) {
    ...
    if (itemSize > MAX_ENCODED_BLOCK_SIZE) {
      throw new CodeError(
        "Cannot send block as after encoding it is over the max message size",
        "ERR_BLOCK_TOO_LARGE",
      );
    }
...
function calculateEncodedBlockPresenceSize(
