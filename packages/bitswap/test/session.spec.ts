...
describe("session", () => {
  ...
  it("should create a session", async () => {
    ...
    components.network.findProviders.withArgs(cid).returns(
      (async function* () {
    ...
    components.wantList.wantSessionBlock
      .withArgs(cid, providers[0].id)
      .resolves({
    ...
    expect(session.providers.length).to.be.lessThan(
      DEFAULT_SESSION_MAX_PROVIDERS,
    );
    expect([...session.providers].map((p) => p.toString())).to.include(
      providers[1].id.toString(),
    );
  ...
  it("should error when creating a session when no peers or providers have the block", async () => {
    ...
    await expect(session.retrieve(cid)).to.eventually.be.rejected.with.property(
      "code",
  ...
  it("should error when creating a session when no providers have the block", async () => {
    ...
    const providers = [
      {
        id: await createEd25519PeerId(),
        multiaddrs: [multiaddr("/ip4/41.41.41.41/tcp/1234")],
      },
    ];
  ...
  it("should exclude non-bitswap providers from the session", async () => {
    ...
    const providers = [
      ...
      {
        id: await createEd25519PeerId(),
        multiaddrs: [multiaddr("/ip4/41.41.41.41/tcp/1235")],
      },
    ];
