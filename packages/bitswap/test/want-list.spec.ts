...
describe("wantlist", () => {
  ...
  it("should want blocks", async () => {
    ...
    expect([...sentMessage.wantlist.values()]).to.have.deep.nested.property(
      "[0].cid",
      cid.bytes,
    );
    expect([...sentMessage.wantlist.values()]).to.have.nested.property(
      "[0].wantType",
      WantType.WantBlock,
    );
    expect([...sentMessage.wantlist.values()]).to.have.nested.property(
  ...
  it("should not send session block wants to non-session peers", async () => {
    ...
    await expect(
      wantList.wantSessionBlock(cid, sessionPeer, {
        signal: AbortSignal.timeout(500),
      }),
    ).to.eventually.be.rejected.with.property("code", "ABORT_ERR");
