...
describe("bitswap", () => {
  ...
  describe("want", () => {
    it("should want a block that is available on the network", async () => {
      ...
      bitswap.network.safeDispatchEvent<BitswapMessageEventDetail>(
        "bitswap:message",
        {
          detail: {
            peer: remotePeer,
    ...
    it("should abort wanting a block that is not available on the network", async () => {
      ...
      await expect(p).to.eventually.be.rejected.with.property(
        "code",
        "ABORT_ERR",
      );
    });
    ...
    it("should notify peers we have a block", async () => {
      const receivedBlockSpy = Sinon.spy(
        bitswap.peerWantLists,
        "receivedBlock",
      );
  ...
  describe("peer wantlist", () => {
    it("should return a peer wantlist", async () => {
      ...
      expect(
        bitswap.getPeerWantlist(remotePeer)?.map((entry) => entry.cid),
      ).to.deep.equal([cid]);
    });
  });
});
