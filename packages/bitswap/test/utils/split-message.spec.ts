...
async function createBitswapBlock (size = 1024): Promise<{ cid: CID, block: Block }> {
...
async function createBlockPresence (size = 1024): Promise<{ cid: CID, presence: BlockPresence }> {
...
async function createWant (size = 1024): Promise<{ cid: CID, entry: WantlistEntry }> {
  const { cid } = await createBlock(size)
...
describe('split-message', () => {
  ...
  it('should split a big message', async () => {
    const input = new QueuedBitswapMessage()
    const block1 = await createBitswapBlock(DEFAULT_MAX_OUTGOING_MESSAGE_SIZE / 2)
    const block2 = await createBitswapBlock(DEFAULT_MAX_OUTGOING_MESSAGE_SIZE / 2)
