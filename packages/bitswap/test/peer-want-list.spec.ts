...
describe("peer-want-lists", () => {
  ...
  it("should replace the wantlist for a peer when the full list is received", async () => {
    ...
    expect(entries?.map((entry) => entry.cid.toString())).to.include(
      cid1.toString(),
    );
    ...
    expect(entries?.map((entry) => entry.cid.toString())).to.not.include(
      cid1.toString(),
    );
    expect(entries?.map((entry) => entry.cid.toString())).to.include(
      cid2.toString(),
  ...
  it("should send requested blocks to peer", async () => {
    ...
    expect([...message.blocks.values()][0].prefix).to.equalBytes(
      ve([
        cid.version,
        cid.code,
        cid.multihash.code,
  ...
  it("should send requested block presences to peer", async () => {
    ...
    const block = Uint8Array.from(
      new Array(DEFAULT_MAX_SIZE_REPLACE_HAS_WITH_BLOCK + 1),
    );
    ...
    expect([...message.blockPresences.values()][0].cid).to.equalBytes(
      cid.bytes,
    );
    expect([...message.blockPresences.values()][0].type).to.equal(
      BlockPresenceType.HaveBlock,
      "should have sent HaveBlock presence",
    );
  });
  ...
  it("should send requested lack of block presences to peer", async () => {
    ...
    expect([...message.blockPresences.values()][0].type).to.equal(
      BlockPresenceType.DontHaveBlock,
      "should have sent DontHaveBlock presence",
    );
  });
  ...
  it("should return peers with want lists", async () => {
    ...
    expect(wantLists.peers().map((p) => p.toString())).to.include(
      remotePeer.toString(),
    );
  });
  ...
  it("should send requested blocks to peer when they are received", async () => {
    ...
    expect(
      wantLists.wantListForPeer(remotePeer)?.map((e) => e.cid.toString()),
    ).to.include(cid.toString());
