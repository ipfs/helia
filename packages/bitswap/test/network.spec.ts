...
describe("network", () => {
  ...
  it("should emit a bitswap:message event when receiving an incoming message", async () => {
    ...
    const messageEventPromise = pEvent<
      "bitswap:message",
      CustomEvent<{ peer: PeerId; message: BitswapMessage }>
    >(network, "bitswap:message");
  ...
  it("should find providers", async () => {
    ...
    components.routing.findProviders.withArgs(cid).returns(
      (async function* () {
        yield* providers;
      })(),
    );
    ...
    expect(output[0].multiaddrs[0].toString()).to.equal(
  ...
  it("should ignore providers with only transient addresses", async () => {
    ...
    const providers = [
      {
        id: peerId,
        multiaddrs: [multiaddr("/ip4/127.0.0.1/tcp/4001/p2p-circuit")],
      },
    ];
    ...
    components.libp2p.isDialable.withArgs(
      matchMultiaddr(providers[0].multiaddrs[0]),
      {
  ...
  it("should send a message", async () => {
    ...
    components.libp2p.dialProtocol
      .withArgs(peerId, BITSWAP_120)
      .resolves(remoteStream);
  ...
  it("should merge messages sent to the same peer", async () => {
    ...
    void network.sendMessage(slowPeer, messageC).catch(() => {});
    ...
    expect(components.libp2p.dialProtocol).to.have.property(
      "callCount",
      2,
      "made too many dials",
    );
