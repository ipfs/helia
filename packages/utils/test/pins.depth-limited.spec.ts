    ...
    describe(`depth ${i}`, () => {
      ...
      it(`pins a block to depth ${i}`, async () => {
        ...
        for (const [name, node] of Object.entries(dag)) {
          if (node.level <= i) {
            await expect(helia.pins.isPinned(node.cid)).to.eventually.be.true(
              `did not pin ${name}`,
            );
          } else {
            await expect(helia.pins.isPinned(node.cid)).to.eventually.be.false(
              `pinned ${name}`,
            );
          }
      ...
      it(`does not delete a pinned sub-block under level ${i}`, async () => {
        ...
        for (const [name, node] of Object.entries(dag)) {
          if (node.level <= i) {
            await expect(
              helia.blockstore.delete(node.cid),
