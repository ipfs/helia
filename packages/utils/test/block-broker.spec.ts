...
describe("block-broker", () => {
  ...
  beforeEach(async () => {
    ...
    storage = new NetworkedStorage({
      ...
      blockBrokers: [bitswapBlockBroker, gatewayBlockBroker],
      hashers: defaultHashers(),
    });
  });

  afterEach(async () => {
    await stop(storage, blockstore);
  });
  ...
  it("gets many blocks from gatewayBlockBroker when they are not in the blockstore", async () => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      gatewayBlockBroker.retrieve
        .withArgs(cid, Sinon.match.any)
        .resolves(block);
  ...
  it("handles incorrect bytes from a gateway", async () => {
    ...
    try {
      ...
      expect(error.errors[0])
        .to.be.an("error")
        .with.property("code", "ERR_HASH_MISMATCH");
    }
  });
});
