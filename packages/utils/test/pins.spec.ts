...
describe("pins", () => {
  ...
  it("pins a block", async () => {
    const cidV1 = await createAndPutBlock(
      raw.code,
    ...
    await expect(helia.pins.isPinned(cidV1)).to.eventually.be.true(
      "did not pin v1 CID",
    );
    await expect(helia.pins.isPinned(cidV0)).to.eventually.be.true(
      "did not pin v0 CID",
    );
  });
  ...
  it("unpins a block", async () => {
    ...
    await expect(helia.pins.isPinned(cidV1)).to.eventually.be.false(
      "did not unpin v1 CID",
    );
    await expect(helia.pins.isPinned(cidV0)).to.eventually.be.false(
      "did not unpin v0 CID",
    );
  });

  it("does not delete a pinned block", async () => {
    const cid = await createAndPutBlock(
      raw.code,
      Uint8Array.from([0, 1, 2, 3]),
      helia.blockstore,
    );
    ...
    await expect(
      helia.blockstore.delete(cid),
    ).to.eventually.be.rejected.with.property("message", "CID was pinned");
  });
  ...
  it("lists pins directly", async () => {
    const cid1 = await createAndPutBlock(
      raw.code,
      Uint8Array.from([0, 1, 2, 3]),
      helia.blockstore,
    );
    const cid2 = await createAndPutBlock(
      raw.code,
  ...
  it("pins a dag-json block", async () => {
    const cid1 = await createAndPutBlock(
      dagJson.code,
      dagJson.encode({ hello: "world" }),
      helia.blockstore,
    );
    const cid2 = await createAndPutBlock(
      dagJson.code,
      dagJson.encode({ hello: "world", linked: cid1 }),
      helia.blockstore,
    );
  ...
  it("pins a dag-cbor block", async () => {
    ...
    const cid2 = await createAndPutBlock(
      dagCbor.code,
      dagCbor.encode({ hello: "world", linked: cid1 }),
      helia.blockstore,
    );
  ...
  it("pins a dag-pb block", async () => {
    const cid1 = await createAndPutBlock(
      dagPb.code,
      dagPb.encode({ Data: Uint8Array.from([0, 1, 2, 3, 4]), Links: [] }),
      helia.blockstore,
    );
    const cid2 = await createAndPutBlock(
      dagPb.code,
      dagPb.encode({ Links: [{ Name: "", Hash: cid1, Tsize: 100 }] }),
      helia.blockstore,
    );
