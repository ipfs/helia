...
describe("pins (recursive)", () => {
  ...
  beforeEach(async () => {
    ...
    helia = await createHelia({
      blockstore,
      blockBrokers: [],
      dagWalkers: [dagWalker(codec, dag)],
    });
  });
    ...
    for (const [name, node] of Object.entries(dag)) {
      for (const cid of node.links) {
        await expect(
          ...
          "message",
          "CID was pinned",
          `allowed deleting pinned block ${name}`,
        );
      }
    }
  });
  ...
  it("should not re-pin blocks pinned during an interrupted pinning operation", async () => {
    ...
    ).to.eventually.be.rejected.with.property("message", "Urk!");

    // we're not actually done. We simulated a crash
    expect(output.done).to.be.false();

    // we pinned the first 5 CIDs in the DAG
    expect(firstTryEvents.map((evt) => evt.detail.toString())).to.deep.equal([
    ...
    const pin = await all(
      helia.pins.add(dag["level-0"].cid, {
        onProgress: (evt) => {
          if (evt.type === "helia:pin:add") {
    ...
    expect(secondTryEvents).to.have.lengthOf(
      pin.length - firstTryEvents.length,
    );

    // these are the rest of the CIDs in the pinned DAG
    expect(secondTryEvents.map((evt) => evt.detail.toString())).to.deep.equal([
