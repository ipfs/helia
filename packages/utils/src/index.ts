...
export {
  AbstractSession,
  type AbstractCreateSessionOptions,
} from "./abstract-session.js";
          ...
          (async function* (): AsyncGenerator<CID> {
            for await (const { cid } of blockstore.getAll()) {
              try {
                ...
                options.onProgress?.(
                  new CustomProgressEvent<CID>("helia:gc:deleted", cid),
                );
              } catch (err: any) {
                helia.log.error("Error during gc", err);
                options.onProgress?.(
                  new CustomProgressEvent<Error>("helia:gc:error", err),
                );
              }
            }
          })(),
