...
export class Routing implements RoutingInterface, Startable {
  ...
  constructor(components: RoutingComponents, init: RoutingInit) {
    ...
    this.providerLookupConcurrency =
      init.providerLookupConcurrency ?? DEFAULT_PROVIDER_LOOKUP_CONCURRENCY;
  }
  ...
  async *findProviders(
    ...
    for await (const peer of merge(
      queue.toGenerator(),
      ...supports(this.routers, "findProviders").map((router) =>
        router.findProviders(key, options),
      ),
    )) {
          ...
          .add(
            async () => {
              try {
                ...
                this.log.error(
                  "could not load multiaddrs for peer %p",
                  peer.id,
                  err,
                );
                return null;
  ...
  async provide(key: CID, options: AbortOptions = {}): Promise<void> {
    ...
    await Promise.all(
      supports(this.routers, "provide").map(async (router) => {
  ...
  async put(
    ...
    await Promise.all(
      supports(this.routers, "put").map(async (router) => {
  ...
  async get(key: Uint8Array, options?: AbortOptions): Promise<Uint8Array> {
    return Promise.any(
      supports(this.routers, "get").map(async (router) => {
  ...
  async findPeer(id: PeerId, options?: RoutingOptions): Promise<PeerInfo> {
    ...
    const self = this;
    const source = merge(
      ...supports(this.routers, "findPeer").map((router) =>
        (async function* () {
          try {
  ...
  async *getClosestPeers(
    ...
    for await (const peer of merge(
      ...supports(this.routers, "getClosestPeers").map((router) =>
        router.getClosestPeers(key, options),
      ),
    )) {
      if (peer == null) {
        continue;
...
function supports<Operation extends keyof Routing>(
  routers: any[],
  key: Operation,
): Array<Pick<Routing, Operation>> {
