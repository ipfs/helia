...
export async function assertDatastoreVersionIsCurrent(
  datastore: Datastore,
): Promise<void> {
  if (!(await datastore.has(DS_VERSION_KEY))) {
    await datastore.put(
      DS_VERSION_KEY,
      uint8ArrayFromString(`${CURRENT_VERSION}`),
    );
  ...
  if (version !== CURRENT_VERSION) {
    ...
    throw new Error(
      "Unknown datastore version, a datastore migration may be required",
    );
  }
}
