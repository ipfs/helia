...
class Storage implements Blockstore {
  ...
  async put(
    ...
    if (await this.child.has(cid, options)) {
      options.onProgress?.(
        new CustomProgressEvent<CID>("blocks:put:duplicate", cid),
      );
      return cid;
    }

    options.onProgress?.(
      new CustomProgressEvent<CID>("blocks:put:providers:notify", cid),
    );

    await Promise.all(
      this.components.blockBrokers.map(async (broker) =>
        broker.announce?.(cid, block, options),
      ),
    );

    options.onProgress?.(
      new CustomProgressEvent<CID>("blocks:put:blockstore:put", cid),
    );
   ...
   */
  async *putMany(
    blocks: AwaitIterable<{ cid: CID; block: Uint8Array }>,
    ...
    const missingBlocks = filter(blocks, async ({ cid }): Promise<boolean> => {
      ...
      if (has) {
        options.onProgress?.(
          new CustomProgressEvent<CID>("blocks:put-many:duplicate", cid),
        );
      }
    ...
    const notifyEach = forEach(
      missingBlocks,
      async ({ cid, block }): Promise<void> => {
        options.onProgress?.(
          new CustomProgressEvent<CID>("blocks:put-many:providers:notify", cid),
        );
        await Promise.all(
          this.components.blockBrokers.map(async (broker) =>
            broker.announce?.(cid, block, options),
    ...
    options.onProgress?.(
      new CustomProgressEvent("blocks:put-many:blockstore:put-many"),
    );
  ...
  async get(
    cid: CID,
    options: GetOfflineOptions &
      AbortOptions &
      ProgressOptions<GetBlockProgressEvents> = {},
  ): Promise<Uint8Array> {
    if (options.offline !== true && !(await this.child.has(cid, options))) {
      // we do not have the block locally, get it from a block provider
      options.onProgress?.(
        new CustomProgressEvent<CID>("blocks:get:providers:get", cid),
      );
      const block = await raceBlockRetrievers(
        cid,
        this.components.blockBrokers,
        this.hashers[cid.multihash.code],
        {
          ...options,
          log: this.log,
        },
      ...
      options.onProgress?.(
        new CustomProgressEvent<CID>("blocks:get:blockstore:put", cid),
      );
      await this.child.put(cid, block, options);

      // notify other block providers of the new block
      options.onProgress?.(
        new CustomProgressEvent<CID>("blocks:get:providers:notify", cid),
      );
      await Promise.all(
        this.components.blockBrokers.map(async (broker) =>
          broker.announce?.(cid, block, options),
    ...
    options.onProgress?.(
      new CustomProgressEvent<CID>("blocks:get:blockstore:get", cid),
    );
   ...
   */
  async *getMany(
    cids: AwaitIterable<CID>,
    options: GetOfflineOptions &
      AbortOptions &
      ProgressOptions<GetManyBlocksProgressEvents> = {},
  ): AsyncIterable<Pair> {
    options.onProgress?.(
      new CustomProgressEvent("blocks:get-many:blockstore:get-many"),
    );

    yield* this.child.getMany(
      forEach(cids, async (cid): Promise<void> => {
        if (options.offline !== true && !(await this.child.has(cid, options))) {
          // we do not have the block locally, get it from a block provider
          options.onProgress?.(
            new CustomProgressEvent<CID>("blocks:get-many:providers:get", cid),
          );
          const block = await raceBlockRetrievers(
            cid,
            this.components.blockBrokers,
          ...
          options.onProgress?.(
            new CustomProgressEvent<CID>("blocks:get-many:blockstore:put", cid),
          );
          await this.child.put(cid, block, options);

          // notify other block providers of the new block
          options.onProgress?.(
            new CustomProgressEvent<CID>(
              "blocks:get-many:providers:notify",
              cid,
            ),
          );
          await Promise.all(
  ...
  async delete(
    ...
    options.onProgress?.(
      new CustomProgressEvent<CID>("blocks:delete:blockstore:delete", cid),
    );
  ...
  async *deleteMany(
    ...
    options.onProgress?.(
      new CustomProgressEvent("blocks:delete-many:blockstore:delete-many"),
    );
    yield* this.child.deleteMany(
      (async function* (): AsyncGenerator<CID> {
  ...
  async *getAll(
    ...
    options.onProgress?.(
      new CustomProgressEvent("blocks:get-all:blockstore:get-many"),
    );
...
class SessionStorage extends Storage implements SessionBlockstore {
  ...
  constructor(components: StorageComponents, init: SessionStorageInit) {
    ...
    this.log = components.logger.forComponent(
      `helia:session-storage:${init.root}`,
    );
  }
...
function isRetrievingBlockBroker(
  broker: BlockBroker,
): broker is Required<Pick<BlockBroker, "retrieve">> {
  return typeof broker.retrieve === "function";
}

export const getCidBlockVerifierFunction = (
  cid: CID,
  hasher: MultihashHasher,
): Required<BlockRetrievalOptions>["validateFn"] => {
  if (hasher == null) {
    throw new CodeError(
      `No hasher configured for multihash code 0x${cid.multihash.code.toString(16)}, please configure one. You can look up which hash this is at https://github.com/multiformats/multicodec/blob/master/table.csv`,
      "ERR_UNKNOWN_HASH_ALG",
    );
  }
  ...
  return async (block: Uint8Array): Promise<void> => {
    ...
    if (!uint8ArrayEquals(hash.digest, cid.multihash.digest)) {
      ...
      throw new CodeError(
        "Hash of downloaded block did not match multihash from passed CID",
        "ERR_HASH_MISMATCH",
      );
    }
  };
 ...
 * found.
 */
async function raceBlockRetrievers(
  cid: CID,
  blockBrokers: BlockBroker[],
  hasher: MultihashHasher,
    ...
    return await Promise.any(
      retrievers.map(async (retriever) => {
        try {
          ...
          options.log.error(
            "could not retrieve verified block for %c",
            cid,
            err,
          );
          throw err;
