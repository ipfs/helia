...
export abstract class AbstractSession<
    Provider,
    RetrieveBlockProgressEvents extends ProgressEvent,
  >
  extends TypedEventEmitter<BlockstoreSessionEvents<Provider>>
  implements BlockBroker<RetrieveBlockProgressEvents>
{
  ...
  constructor(
    components: AbstractSessionComponents,
    init: AbstractCreateSessionOptions,
  ) {
    super();
  ...
  async retrieve(
    cid: CID,
    options: BlockRetrievalOptions<RetrieveBlockProgressEvents> = {},
  ): Promise<Uint8Array> {
    ...
    if (this.providers.length === 0) {
      ...
      if (this.intialPeerSearchComplete == null) {
        ...
        this.intialPeerSearchComplete = this.findProviders(
          cid,
          this.minProviders,
          options,
        );
    ...
    const queue = new Queue<
      Uint8Array,
      { provider: Provider; priority?: number } & AbortOptions
    >({
    ...
    queue.addEventListener("failure", (evt) => {
      this.log.error(
        "error querying provider %o, evicting from session",
        evt.detail.job.options.provider,
        evt.detail.error,
      );
    ...
    queue.addEventListener("idle", () => {
      ...
      Promise.resolve()
        .then(async () => {
          this.log(
            "no session peers had block for for %c, finding new providers",
            cid,
          );
  ...
  evict(provider: Provider): void {
    ...
    const index = this.providers.findIndex((prov) =>
      this.equals(prov, provider),
    );
  ...
  private async findProviders(
    ...
    void Promise.resolve()
      .then(async () => {
        this.log(
          "finding %d-%d new provider(s) for %c",
          count,
          this.maxProviders,
          cid,
        );
        ...
        if (found < count) {
          throw new CodeError(
            `Found ${found} of ${count} ${this.name} providers for ${cid}`,
            "ERR_INSUFFICIENT_PROVIDERS_FOUND",
          );
        }
      })
      .catch((err) => {
        this.log.error(
          "error searching routing for potential session peers for %c",
          cid,
          err.errors ?? err,
        );
        deferred.reject(err);
