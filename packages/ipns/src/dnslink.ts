...
async function recursiveResolveDnslink(
  domain: string,
  depth: number,
  dns: DNS,
  log: Logger,
  options: ResolveDNSLinkOptions = {},
): Promise<DNSLinkResult> {
  if (depth === 0) {
  ...
  const txtRecords = txtRecordsResponse.Answer.sort((a, b) =>
  ...
  for (const answer of txtRecords) {
    try {
      ...
      if (protocol === "ipfs") {
        ...
        return await recursiveResolveDomain(
          domainOrCID,
          depth - 1,
          dns,
          log,
        ...
        log(
          'unknown protocol "%s" in DNSLink record for domain: %s',
          protocol,
          domain,
        );
        continue;
      ...
      log.error(
        "could not parse DNS link record for domain %s, %s",
        domain,
        answer.data,
        err,
      );
  ...
  const cnameRecords = cnameRecordsResponse.Answer.sort((a, b) =>
    a.data.localeCompare(b.data),
  );
  ...
  for (const cname of cnameRecords) {
    try {
      ...
      log.error(
        "domain %s cname %s had no DNSLink records",
        domain,
        cname.data,
        err,
      );
  ...
  throw new CodeError(
    `No DNSLink records found for domain: ${domain}`,
    "ERR_DNSLINK_NOT_FOUND",
  );
}

async function recursiveResolveDomain(
  domain: string,
  depth: number,
  dns: DNS,
  log: Logger,
  ...
  try {
    ...
    if (
      err.code !== "ENOTFOUND" &&
      err.code !== "ERR_DNSLINK_NOT_FOUND" &&
      err.code !== "ENODATA"
    ) {
      throw err;
    }
...
export async function resolveDNSLink(
  ...
  return recursiveResolveDomain(
    domain,
    options.maxRecursiveDepth ?? MAX_RECURSIVE_DEPTH,
    dns,
    log,
    options,
  );
