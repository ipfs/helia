  ...
  return {
    async put(
      ...
      try {
        ...
        options.onProgress?.(
          new CustomProgressEvent("ipns:routing:datastore:put"),
        );
        await datastore.put(key, record.serialize(), options);
      } catch (err: any) {
        options.onProgress?.(
          new CustomProgressEvent<Error>("ipns:routing:datastore:error", err),
        );
        throw err;
      }
    },
    async get(
      ...
      try {
        ...
        options.onProgress?.(
          new CustomProgressEvent("ipns:routing:datastore:get"),
        );
