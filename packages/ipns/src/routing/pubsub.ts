...
export type PubSubProgressEvents =
  | ProgressEvent<
      "ipns:pubsub:publish",
      { topic: string; result: PublishResult }
    >
...
class PubSubRouting implements IPNSRouting {
  ...
  async #processPubSubMessage(message: Message): Promise<void> {
    ...
    if (message.type !== "signed") {
      log.error(
        "unsigned message received, this module can only work with signed messages",
      );
      return;
    }
  ...
  async put(
    ...
    try {
      ...
      log(
        "published record on topic %s to %d recipients",
        topic,
        result.recipients,
      );
      options.onProgress?.(
        new CustomProgressEvent("ipns:pubsub:publish", { topic, result }),
      );
    } catch (err: any) {
      options.onProgress?.(
        new CustomProgressEvent<Error>("ipns:pubsub:error", err),
      );
      throw err;
    }
  }
  ...
  async get(
    ...
    try {
      ...
      if (!this.pubsub.getTopics().includes(topic)) {
        ...
        options.onProgress?.(
          new CustomProgressEvent("ipns:pubsub:subscribe", { topic }),
        );
      }
...
function topicToKey(topic: string): Uint8Array {
  if (topic.substring(0, PUBSUB_NAMESPACE.length) !== PUBSUB_NAMESPACE) {
    throw new CodeError(
      "topic received is not from a record",
      "ERR_TOPIC_IS_NOT_FROM_RECORD_NAMESPACE",
    );
  }
