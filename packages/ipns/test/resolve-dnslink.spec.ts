...
describe("resolveDNSLink", () => {
  ...
  it("should resolve a domain", async () => {
    dns.query.withArgs("_dnslink.foobar.baz").resolves(
      dnsResponse([
        {
  ...
  it("should retry without `_dnslink.` on a domain", async () => {
    dns.query
      .withArgs("_dnslink.foobar.baz")
      .rejects(new CodeError("Not found", "ENOTFOUND"));
    dns.query.withArgs("foobar.baz").resolves(
      dnsResponse([
        {
          name: "foobar.baz.",
          TTL: 60,
  ...
  it("should support trailing slash in returned dnslink value", async () => {
    ...
    expect(result.cid.toString()).to.equal(
      ...
      "doesn't support trailing slashes",
    );
  });
  ...
  it("should follow CNAMES to delegated DNSLink domains", async () => {
    ...
    dns.query.withArgs("_dnslink.delegated.foobar.baz").resolves(
      dnsResponse([
        {
