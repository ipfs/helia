...
export interface HeliaLibp2p<T extends Libp2p = Libp2p<DefaultLibp2pServices>>
  extends Helia {
  libp2p: T;
}
 ...
 */
export async function createHelia<T extends Libp2p>(
  init: Partial<HeliaInit<T>>,
): Promise<HeliaLibp2p<T>>;
export async function createHelia(
  init?: Partial<HeliaInit<Libp2p<DefaultLibp2pServices>>>,
): Promise<HeliaLibp2p<Libp2p<DefaultLibp2pServices>>>;
export async function createHelia(
  init: Partial<HeliaInit> = {},
): Promise<HeliaLibp2p> {
  ...
  const helia = new HeliaP2P({
    ...
    datastore,
    blockstore,
    blockBrokers: init.blockBrokers ?? [trustlessGateway(), bitswap()],
