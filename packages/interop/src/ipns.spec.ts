  ...
  describe(`@helia/ipns - default routing with ${type} keys`, () => {
    ...
    async function createNodes(resolver: "kubo" | "helia"): Promise<void> {
      ...
      while (true) {
        ...
        if (
          resolver === "kubo" &&
          closest.equals(peerIdFromString((await kubo.api.id()).id.toString()))
        ) {
          break;
        }
      ...
      await waitFor(
        async () => {
          ...
          for await (const event of helia.libp2p.services.dht.findPeer(
            peerIdFromString((await kubo.api.id()).id.toString()),
          )) {
      ...
      await waitFor(
        async () => {
          ...
          for await (const event of kubo.api.routing.findPeer(
            helia.libp2p.peerId,
          )) {
            if (event.name === "FINAL_PEER") {
              found = true;
    ...
    it("should publish on kubo and resolve on helia", async function () {
      ...
      const response = await fetch(
        `http://${config.host}:${config.port}${config.pathname}/key/import?arg=${keyName}`,
        {
          method: "POST",
          body,
        },
      ...
      const oneHourNS = BigInt(60 * 60 * 1e9);
