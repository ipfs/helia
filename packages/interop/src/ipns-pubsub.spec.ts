  ...
  .forEach((keyType) => {
    describe(`@helia/ipns - pubsub routing with ${keyType} keys`, () => {
      ...
      it("should publish on helia and resolve on kubo", async () => {
        ...
        const peerId =
          await helia.libp2p.services.keychain.exportPeerId(keyName);
        ...
        await expect(name.publish(peerId, cid))
      ...
      it("should publish on kubo and resolve on helia", async function () {
        ...
        const subscriptionName = `/record/${uint8ArrayToString(
          uint8ArrayConcat([uint8ArrayFromString("/ipns/"), peerId.toBytes()]),
          "base64url",
        )}`;
        ...
        await waitFor(
          async () => {
            return helia.libp2p.services.pubsub
              .getTopics()
        ...
        await waitFor(
          async () => {
            ...
            return peers
              .map((p) => p.toString())
              .includes(helia.libp2p.peerId.toString());
          },
          {
            timeout: 30000,
            message:
              "Kubo did not see that Helia was registered for record updates",
          },
        );
