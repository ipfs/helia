...
describe("trustless-gateway-block-broker", () => {
  ...
  beforeEach(async () => {
    ...
    badGatewayPeer = {
      ...
      multiaddrs: [uriToMultiaddr(process.env.BAD_TRUSTLESS_GATEWAY ?? "")],
    };
    goodGatewayPeer = {
      id: await createEd25519PeerId(),
  ...
  it("tries all gateways before failing", async () => {
    ...
    const cid = CID.parse(
      "bafkreihrn5vx6hgqqltsfbhyr4fqlfu47icddprfubkqrztm6yft5iwpmm",
    );
  ...
  it("creates a session", async () => {
    routing.findProviders.returns(
      (async function* () {
        // non-http provider
        yield {
          id: await createEd25519PeerId(),
          multiaddrs: [multiaddr("/ip4/132.32.25.6/tcp/1234")],
        };
        // expired peer info
        yield {
          id: await createEd25519PeerId(),
    ...
    await expect(sessionBlockstore?.retrieve?.(cid)).to.eventually.deep.equal(
      expectedBlock,
    );
  });
  ...
  it("does not trigger new network requests if the same cid request is in-flight", async function () {
    ...
    const trustlessGateway = new TrustlessGateway(
      process.env.TRUSTLESS_GATEWAY,
      defaultLogger(),
    );

    // Call getRawBlock multiple times with the same CID
    const promises = Array.from({ length: 10 }, async () =>
      trustlessGateway.getRawBlock(cid),
    );
