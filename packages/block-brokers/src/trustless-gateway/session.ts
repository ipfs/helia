...
> {
  ...
  constructor(
    ...
    init: CreateTrustlessGatewaySessionOptions,
  ) {
    super(components, {
      ...init,
  ...
  async *findNewProviders(
    cid: CID,
    options: AbortOptions = {},
  ): AsyncGenerator<TrustlessGateway> {
    yield* findHttpGatewayProviders(
      cid,
      this.routing,
      this.logger,
      this.allowInsecure,
...
export function createTrustlessGatewaySession(
  components: TrustlessGatewaySessionComponents,
  init: CreateTrustlessGatewaySessionOptions,
