...
export class TrustlessGateway {
  ...
  constructor(url: URL | string, logger: ComponentLogger) {
    ...
    this.log = logger.forComponent(
      `helia:trustless-gateway-block-broker:${this.url.hostname}`,
    );
  }
  ...
  async getRawBlock(cid: CID, signal?: AbortSignal): Promise<Uint8Array> {
    ...
    try {
      let pendingResponse: Promise<Uint8Array> | undefined =
        this.#pendingResponses.get(blockId);
      if (pendingResponse == null) {
        ...
        pendingResponse = fetch(gwUrl.toString(), {
          ...
          if (!res.ok) {
            this.#errors++;
            throw new Error(
              `unable to fetch raw block for CID ${cid} from gateway ${this.url}`,
            );
          }
          this.#successes++;
      ...
      if (signal?.aborted === true) {
        throw new Error(
          `fetching raw block for CID ${cid} from gateway ${this.url} was aborted`,
        );
      }
      this.#errors++;
