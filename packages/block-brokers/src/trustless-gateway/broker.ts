...
export interface CreateTrustlessGatewaySessionOptions
  extends CreateSessionOptions<TrustlessGatewayGetBlockProgressEvents> {
  /**
...
export class TrustlessGatewayBlockBroker
  implements BlockBroker<TrustlessGatewayGetBlockProgressEvents>
{
  ...
  constructor(
    ...
    this.log = components.logger.forComponent(
      "helia:trustless-gateway-block-broker",
    );
  ...
  async retrieve(
    cid: CID,
    options: BlockRetrievalOptions<TrustlessGatewayGetBlockProgressEvents> = {},
  ): Promise<Uint8Array> {
    const aggregateErrors: Error[] = [];

    for await (const gateway of findHttpGatewayProviders(
      cid,
      this.routing,
      this.logger,
      this.allowInsecure,
      ...
      try {
        ...
        try {
          ...
          this.log.error(
            "failed to validate block for %c from %s",
            cid,
            gateway.url,
            err,
          );
        ...
        this.log.error(
          "failed to get block for %c from %s",
          cid,
          gateway.url,
          err,
        );
        ...
        if (err instanceof Error) {
          ...
          aggregateErrors.push(
            new Error(
              `Unable to fetch raw block for CID ${cid} from gateway ${gateway.url}`,
            ),
          );
        }
        ...
        if (options.signal?.aborted === true) {
          this.log.trace(
            "request aborted while fetching raw block for CID %c from gateway %s",
            cid,
            gateway.url,
          );
          break;
    ...
    if (aggregateErrors.length > 0) {
      throw new AggregateError(
        aggregateErrors,
        `Unable to fetch raw block for CID ${cid} from any gateway`,
      );
    } else {
      throw new Error(
        `Unable to fetch raw block for CID ${cid} from any gateway`,
  ...
  createSession(
    options: CreateTrustlessGatewaySessionOptions = {},
