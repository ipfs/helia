...
class BitswapBlockBroker
  implements
    BlockBroker<BitswapWantBlockProgressEvents, BitswapNotifyProgressEvents>,
    Startable
{
  ...
  constructor(components: BitswapComponents, init: BitswapInit = {}) {
    ...
    this.bitswap = createBitswap(components, {
      hashLoader: {
        getHasher: async (
          codecOrName: string | number,
        ): Promise<MultihashHasher<number>> => {
  ...
  async announce(
    cid: CID,
    block: Uint8Array,
    options?: BlockAnnounceOptions<BitswapNotifyProgressEvents>,
  ): Promise<void> {
  ...
  async retrieve(
    cid: CID,
    options: BlockRetrievalOptions<BitswapWantBlockProgressEvents> = {},
  ): Promise<Uint8Array> {
    return this.bitswap.want(cid, options);
  }

  createSession(
    options?: CreateSessionOptions<BitswapWantBlockProgressEvents>,
  ): BlockBroker<BitswapWantBlockProgressEvents, BitswapNotifyProgressEvents> {
...
export function bitswap(
  init: BitswapInit = {},
): (components: BitswapComponents) => BlockBroker {
