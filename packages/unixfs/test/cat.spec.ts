...
describe("cat", () => {
  ...
  it("refuses to read a directory", async () => {
    await expect(
      drain(fs.cat(emptyDirCid)),
    ).to.eventually.be.rejected.with.property("code", "ERR_NOT_A_FILE");
  });
  ...
  it("should only load blocks necessary to traverse a HAMT", async () => {
    const [, scriptFile, styleFile, imageFile, dir] = await all(
      fs.addAll(
        [
          {
            path: "index.html",
    ...
    await drain(
      blockstore.deleteMany([scriptFile.cid, styleFile.cid, imageFile.cid]),
    );
