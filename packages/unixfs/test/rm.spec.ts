...
describe("rm", () => {
  ...
  it("removes a sharded directory inside a normal directory", async () => {
    ...
    await expect(fs.stat(containingDirCid)).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
    await expect(
      fs.stat(containingDirCid, {
        path: dirName,
      }),
    ...
    await expect(
      fs.stat(updatedContainingDirCid, {
        path: dirName,
      }),
    ).to.eventually.be.rejected.with.property("code", "ERR_NOT_FOUND");
  });
  ...
  it("removes a sharded directory inside a sharded directory", async () => {
    ...
    const dirName = `subdir-${Math.random()}`;
    const containingDirCid = await fs.cp(
      shardedDirCid,
      otherShardedDirCid,
      dirName,
    );
    ...
    expect(updatedContainingDirCid.toString()).to.equal(
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a file and removing the file means the root node is below the shard threshold", async function () {
    ...
    await expect(fs.stat(importerCid)).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
    ...
    const dirWithFileCid = await fs.cp(
      fileCid,
      containingDirCid,
      "file-2.txt",
      {
    ...
    await expect(fs.stat(dirWithFileCid)).to.eventually.have.nested.property(
      "unixfs.type",
      "hamt-sharded-directory",
    );
    ...
    await expect(fs.stat(dirWithoutFileCid)).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a subshard", async function () {
    ...
    await expect(fs.stat(finalDirCid)).to.eventually.have.nested.property(
      "unixfs.type",
      "hamt-sharded-directory",
    );

    expect(finalDirCid).to.eql(
      importerCid,
      "removing a file from the imported dir not result in the same CID",
    );
  });
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a subshard of a subshard", async function () {
    let { containingDirCid, fileName, importerCid } =
