...
describe("addAll", () => {
  ...
  it("adds a stream of files", async () => {
    ...
    expect(output[0].cid.toString()).to.equal(
      "bafkreiaixnpf23vkyecj5xqispjq5ubcwgsntnnurw2bjby7khe4wnjihu",
    );
    expect(output[1].cid.toString()).to.equal(
      "bafkreidmuy2n45xj3cdknzprtzo2uvgm3hak6mzy5sllxty457agsftd34",
    );
  });
  ...
  it("recursively adds a directory", async function () {
    ...
    const res = await last(
      fs.addAll(globSource("./test/fixtures", "files/**/*")),
    );

    expect(res?.cid.toString()).to.equal(
      "bafybeievhllpjjjbyg53g74wcl5hckdccjjj7zgtexqcacjegoduegnkyu",
    );
  });
});
...
describe("addFile", () => {
  ...
  it("adds a file from a URL", async () => {
    const cid = await fs.addFile(
      urlSource(
        new URL(`${process.env.ECHO_SERVER}/download?data=hello-world`),
      ),
    );

    expect(cid.toString()).to.equal(
      "bafkreifpuj5ujvb3aku75ja5cphnylsac3h47b6f3p4zbzmtm2nkrtrinu",
    );
  });
});
