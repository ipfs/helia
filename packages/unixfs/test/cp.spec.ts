...
describe("cp", () => {
  ...
  it("refuses to copy files to an unreadable node", async () => {
    ...
    await expect(
      fs.cp(source, target, "foo"),
    ).to.eventually.be.rejected.with.property("code", "ERR_NOT_A_DIRECTORY");
  });
  ...
  it("refuses to copy files to an existing file in a sharded directory", async () => {
    ...
    await expect(
      fs.cp(file.cid, shardedDirCid, file.name),
    ).to.eventually.be.rejected.with.property("code", "ERR_ALREADY_EXISTS");
