...
describe("mkdir", () => {
  ...
  it("refuses to create a directory that already exists", async () => {
    ...
    await expect(fs.mkdir(dirCid, path))
  ...
  it("creates a nested directory with a different CID version to the parent", async () => {
    ...
    await expect(fs.stat(dirCid)).to.eventually.have.nested.property(
      "cid.version",
      0,
    );
    await expect(
  ...
  it("makes a directory inside a sharded directory", async () => {
    ...
    await expect(fs.stat(updatedShardCid)).to.eventually.have.nested.property(
      "unixfs.type",
