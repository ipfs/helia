...
function fixtureDir(): string {
  ...
  return Path.resolve(
    Path.join(dirname, "..", "..", "..", "test", "fixtures", "files"),
  );
}
...
describe("glob-source", () => {
  it("single file, relative path", async function () {
    ...
    const result = await all(
      globSource("./test/fixtures/files", "file-0.html"),
    );
  ...
  it("multiple directories", async function () {
    ...
    const result = await all(
      globSource(
        fixtureDir(),
        "{dir/nested-dir,another-dir/another-nested-dir}/**/*",
      ),
    );
  ...
  it("overrides mtime for file with secs/nsecs", async function () {
    ...
    const result = await all(
      globSource(fixture("dir"), "file-1.txt", {
        mtime: { secs: 5n, nsecs: 0 },
      }),
    );

    expect(result).to.have.deep.nested.property("[0].mtime", {
      secs: 5n,
      nsecs: 0,
    });
  });
  ...
  it("overrides mtime for file with UnixFS timespec", async function () {
    ...
    expect(result).to.have.deep.nested.property(
      "[0].mtime",
      toMtime({ Seconds: 5, FractionalNanoseconds: 0 }),
    );
  });
});
