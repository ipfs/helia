...
export async function addLink(
  parent: Directory,
  child: Required<PBLink>,
  blockstore: GetStore & PutStore,
...
const convertToShardedDirectory = async (
  parent: Directory,
  blockstore: PutStore,
  ...
  const result = await createShard(
    blockstore,
    parent.node.Links.map((link) => ({
      name: link.Name ?? "",
...
const addToDirectory = async (
  ...
  if (parent.node.Data == null) {
    throw new InvalidPBNodeError(
      "Parent node with no data passed to addToDirectory",
    );
  }
...
const addToShardedDirectory = async (
  parent: Directory,
  child: Required<PBLink>,
  blockstore: GetStore & PutStore,
  options: AddLinkOptions,
): Promise<AddLinkResult> => {
  const { path, hash } = await recreateShardedDirectory(
    parent.cid,
    child.Name,
    blockstore,
    options,
  ...
  const existingLink = finalSegment.node.Links.find((l) =>
  ...
  if (existingLink != null) {
    ...
    if (existingLink.Name === linkName) {
      ...
      const index = finalSegment.node.Links.findIndex((l) =>
        l.Name?.startsWith(prefix),
      );
