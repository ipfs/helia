...
export async function removeLink(
  parent: Directory,
  name: string,
  blockstore: PutStore & GetStore,
  options: RmLinkOptions,
  ...
  if (meta.type === "hamt-sharded-directory") {
    ...
    const result = await removeFromShardedDirectory(
      parent,
      name,
      blockstore,
      options,
    ...
    if (
      !(await isOverShardThreshold(
        result.node,
        blockstore,
        options.shardSplitThresholdBytes,
        options,
      ))
    ) {
...
const removeFromDirectory = async (
  parent: Directory,
  name: string,
  blockstore: PutStore & GetStore,
...
const removeFromShardedDirectory = async (
  parent: Directory,
  name: string,
  blockstore: PutStore & GetStore,
  options: UpdateHamtDirectoryOptions,
): Promise<{ cid: CID; node: PBNode }> => {
  const { path } = await recreateShardedDirectory(
    parent.cid,
    name,
    blockstore,
    options,
  ...
  const linkName = finalSegment.node.Links.filter(
    (l) => (l.Name ?? "").substring(2) === name,
  )
    .map((l) => l.Name)
    .pop();
  ...
  finalSegment.node.Links = finalSegment.node.Links.filter(
  ...
  if (finalSegment.node.Links.length === 1) {
    ...
    while (true) {
      ...
      nextSegment.node.Links = nextSegment.node.Links.filter(
        (l) => !(l.Name ?? "").startsWith(nextSegment.prefix),
      );
...
const convertToFlatDirectory = async (
  parent: Directory,
  blockstore: PutStore & GetStore,
  options: RmLinkOptions,
): Promise<RemoveLinkResult> => {
  if (parent.node.Data == null) {
    throw new InvalidParametersError(
      "Invalid parent passed to convertToFlatDirectory",
    );
  }
