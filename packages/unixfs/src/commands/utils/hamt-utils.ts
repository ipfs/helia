...
export const createShard = async (
  blockstore: PutStore,
  contents: Array<{ name: string; size: bigint; cid: CID }>,
...
export const updateShardedDirectory = async (
  path: HAMTPath[],
  blockstore: GetStore & PutStore,
  options: PersistOptions,
): Promise<{ cid: CID; node: dagPB.PBNode }> => {
  ...
  for (let i = 0; i < path.length; i++) {
    ...
    if (!isRoot) {
      ...
      if (nextSegment == null) {
        throw new Error(
          "Was not operating on shard root but also had no parent?",
        );
      }

      log(
        "updating link in parent sub-shard with prefix %s",
        nextSegment.prefix,
      );

      nextSegment.node.Links = nextSegment.node.Links.filter(
        (l) => l.Name !== nextSegment.prefix,
      );
      nextSegment.node.Links.push({
        Name: nextSegment.prefix,
        Hash: cid,
        Tsize: segment.node.Links.reduce(
...
export const recreateShardedDirectory = async (
  cid: CID,
  fileName: string,
  blockstore: Pick<Blockstore, "get">,
  options: AbortOptions,
): Promise<{ path: HAMTPath[]; hash: InfiniteHash }> => {
