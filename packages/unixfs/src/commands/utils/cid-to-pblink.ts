...
export async function cidToPBLink(
  ...
  return {
    Name: name,
    Tsize:
      sourceEntry.node instanceof Uint8Array
        ? sourceEntry.node.byteLength
        : dagNodeTsize(sourceEntry.node),
    Hash: cid,
  };
}

function dagNodeTsize(node: PBNode): number {
  const linkSizes = node.Links.reduce(
