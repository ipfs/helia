...
export async function stat(
  ...
  if (result.type === "file") {
    ...
    dagSize = BigInt(
      (result.node.Data?.byteLength ?? 0) +
        result.node.Links.reduce((acc, curr) => acc + (curr.Tsize ?? 0), 0),
    );
...
async function inspectDag(
  cid: CID,
  blockstore: GetStore & HasStore,
  options: AbortOptions,
): Promise<InspectDagResults> {
  const results = {
    localFileSize: 0,
    localDagSize: 0,
    blocks: 0,
      ...
      if (pbNode.Links.length > 0) {
        ...
        if (unixfs.data == null) {
          throw new InvalidPBNodeError(
            `UnixFS node ${cid.toString()} had no data`,
          );
        }
