...
describe("cp", () => {
  ...
  it("refuses to copy files without a source", async () => {
    ...
    await expect(fs.cp()).to.eventually.be.rejected.with(
      "Please supply at least one source",
    );
  });
  ...
  it("refuses to copy files without a destination", async () => {
    // @ts-expect-error invalid args
    await expect(fs.cp("/source")).to.eventually.be.rejected.with(
      "Please supply at least one source",
    );
  });
  ...
  it("refuses to copy an unreadable node", async () => {
    ...
    await expect(
      fs.cp(source, "/foo.txt"),
    ).to.eventually.be.rejected.with.property("code", "ERR_NOT_UNIXFS");
  });

  it("refuses to copy a non-existant file", async () => {
    await expect(
      fs.cp("/foo.txt", "/bar.txt"),
    ).to.eventually.be.rejected.with.property("code", "ERR_DOES_NOT_EXIST");
  });
  ...
  it("refuses to copy files to an existing file", async () => {
    ...
    await expect(fs.cp(path, path)).to.eventually.be.rejected.with.property(
      "code",
      "ERR_ALREADY_EXISTS",
    );
  ...
  it("copies a sharded directory to a normal directory", async () => {
    ...
    await expect(fs.stat("/")).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );

    const subDirStats = await fs.stat(path);
    expect(subDirStats).to.have.nested.property(
      "unixfs.type",
  ...
  it("copies a normal directory to a sharded directory", async () => {
    ...
    await expect(fs.stat(shardedDirPath)).to.eventually.have.nested.property(
      "unixfs.type",
      "hamt-sharded-directory",
    );
    ...
    expect(subDirStats.cid.toString()).to.equal(
  ...
  it("copies a file from a normal directory to a sharded directory", async () => {
    ...
    expect(fileInDirStats.cid.toString()).to.equal(
      "bafybeibyuhrlz5wvmzhn5twibjtpofeek4v6afw3uvk6jkewrdkgivvcea",
    );
  });
  ...
  it("refuses to copy files to an existing file in a sharded directory", async () => {
    ...
    await expect(
      fs.cp(file.cid, `${shardedDirPath}/${file.name}`),
    ).to.eventually.be.rejected.with.property("code", "ERR_ALREADY_EXISTS");
  ...
  it("copies a file to a sharded directory that creates a subshard", async () => {
    const { containingDirCid, fileName, importerCid } =
    ...
    expect(stat.cid.toString()).to.equal(
      containingDirCid.toString(),
      "adding a file to the imported dir did not result in the same CID",
    );
  });
});
