...
describe("rm", () => {
  ...
  it("removes a file", async () => {
    ...
    await expect(fs.stat(filePath)).to.eventually.be.rejected.with.property(
      "code",
      "ERR_DOES_NOT_EXIST",
    );
  });
  ...
  it("removes a directory", async () => {
    ...
    await expect(fs.stat(dirPath)).to.eventually.be.rejected.with.property(
      "code",
      "ERR_DOES_NOT_EXIST",
    );
  });
  ...
  it("removes a sharded directory inside a normal directory", async () => {
    ...
    await expect(fs.stat(dirPath)).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
  ...
  it("removes a sharded directory inside a sharded directory", async () => {
    ...
    await expect(fs.stat(shardedSubDirPath)).to.eventually.have.nested.property(
      "unixfs.type",
      "hamt-sharded-directory",
    );
    ...
    await expect(
      fs.stat(shardedSubDirPath),
    ).to.eventually.be.rejected.with.property("code", "ERR_DOES_NOT_EXIST");

    const shardedDirStats = await fs.stat(shardedDirPath);
    expect(shardedDirStats.cid.toString()).to.equal(
      shardedDirCid.toString(),
      "adding and removing a file from a sharded directory did not result in the original sharded CID",
    );
  });
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a file and removing the file means the root node is below the shard threshold", async function () {
    ...
    await expect(fs.stat("/importer-dir")).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a file", async function () {
    ...
    const dirStats = await fs.stat(dirPath);
    expect(dirStats).to.have.nested.property(
      "unixfs.type",
      "hamt-sharded-directory",
    );
    expect(dirStats.cid.toString()).to.equal(importerCid.toString());
  ...
  it("results in the same hash as a sharded directory created by the importer when removing a subshard", async function () {
    ...
    expect(dirStats.cid.toString()).to.equal(
      importerCid.toString(),
      "removing a file from the imported dir did not result in the same CID",
    );
  });
