...
describe("touch", () => {
  ...
  it("should have default mtime", async () => {
    ...
    await expect(fs.stat(path))
      .to.eventually.have.property("mtime")
      .that.is.undefined();
  ...
  it("should update mtime recursively for a hamt-sharded-directory", async function () {
    ...
    await expect(fs.stat(shardedDirPath))
      .to.eventually.have.nested.property("mtime.secs")
      // no bigint support
      .that.satisfies((s: bigint) => s > seconds);
    ...
    for await (const file of fs.ls(shardedDirPath)) {
      expect(file)
        .to.have.nested.property("unixfs.mtime.secs")
        // no bigint support
        .that.satisfies((s: bigint) => s > seconds);
    }
  });
