...
describe("mkdir", () => {
  ...
  async function testMode(
    ...
    await expect(fs.stat(path)).to.eventually.have.property(
      "mode",
      expectedMode,
    );
  }
  ...
  async function testMtime(mtime: Mtime, expectedMtime: Mtime): Promise<void> {
    ...
    await expect(fs.stat(path)).to.eventually.have.deep.property(
      "mtime",
      expectedMtime,
    );
  }
  ...
  it("creates a nested directory with a different CID version to the parent", async () => {
    ...
    await expect(fs.stat(path)).to.eventually.have.nested.property(
      "cid.version",
      1,
    );
    await expect(fs.stat(subDirectory)).to.eventually.have.nested.property(
      "cid.version",
      0,
    );
  });
  ...
  it("makes a directory inside a sharded directory", async () => {
    ...
    await expect(fs.stat(dirName)).to.eventually.have.nested.property(
      "unixfs.type",
      "directory",
    );
  });
